...
void dfs(int i, int status, unsigned long long val, const string& s, int len_a, const vector<int>& d, unsigned long long a, unsigned long long& min_diff) {
    ...
    if (status == -1) dfs(i + 1, -1, val * 10 + d.back(), s, len_a, d, a, min_diff);
    else if (status == 1) dfs(i + 1, 1, val * 10 + d.front(), s, len_a, d, a, min_diff);
    else {
        for (int x : d) {
            int next_status = 0;
...
void solution(unsigned long long a, int n, vector<int>& d) {
    ...
    if (len_a > 1) {
        ...
        for (int i = 0; i < len_a - 1; i++) val_short = val_short * 10 + d.back();
        update(a, val_short, min_diff);
    }

    int first = d.front();
    if (first == 0 && n > 1) first = d[1];

    unsigned long long val_long = first;
    for (int i = 0; i < len_a; i++) val_long = val_long * 10 + d.front();
